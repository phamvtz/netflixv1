...
function nfDetectPaymentHold(html) {
  ...
  if (/payment[\s\S]{0,40}(?:failed|unsuccessful|declined|issue|problem)/i.test(low)) return true;

  if (/data-uia="[^"]*(?:payment|billing|notification)[^"]*(?:error|issue|hold|pastdue|banner|alert)[^"]*"/i.test(raw)) {
    return true;
  }
  if (/account-overview-page\+[^"]*(?:notification|alert|banner|message)/i.test(raw) &&
      /payment|billing|thanh toán/i.test(compact) &&
      /update|unable|process|hold|failed|continue/i.test(compact)) {
    return true;
  }
  ...
  if (ms && /hold|past.?due|payment|grace|suspended|retry/i.test(ms[1])) return true;
...
module.exports = {
  nfClean, nfIsValidEmail, nfExtractEmailFromHtml, nfJsonValues, nfVisibleLines, nfDetectPaymentHold,
};
